...
try{
	...
	$query = "SELECT c.post_id as post_id, c.user_id as owner_id, c.image_url as image_url, c.is_Priced as is_Priced, c.price as price, c.description as description, c.created_date as created_date, c.last_updated_date as last_updated_date, c.status as status, c.is_private as is_private, c.watermark_id as watermark_id, c.category as category, c.up_count as up_count, c.down_count as down_count, IFNULL(c.upVote, 'N') as upVote from (SELECT post_id, user_id, image_url, is_Priced, price, description, created_date, last_updated_date, status, is_private, watermark_id, category, up_count, down_count, (SELECT ud_flag from updownvotes where user_id = $user_id and post_id = a.post_id) as upVote FROM posts a WHERE a.post_id IN (SELECT post_id from post_tags WHERE tag_id = $tag_id) ) c ORDER BY created_date DESC";
